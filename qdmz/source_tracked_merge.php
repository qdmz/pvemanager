...
foreach ($files_to_merge as $file_path) {
    $source_filename = basename($file_path, '.' . pathinfo($file_path, PATHINFO_EXTENSION)); // 获取不带扩展名的文件名
                ...
                if (($name_col_index != -1 && $curr_name_col_index != -1 && $base_name == $name_value) ||
                    ...
                    for ($j = 0; $j < count($current_col_names); $j++) {
                        if ($j != $curr_name_col_index && $j != $curr_id_col_index) { // 不处理姓名和身份证列
                            $original_col_name = $current_col_names[$j];
                            $new_col_name = $source_filename . '_' . $original_col_name; // 包含来源文件名的新列名
                            
                            // 检查基础数据中是否已存在该列（检查原始列名和带来源的列名）
                            $base_col_index = -1;
                            for ($k = 0; $k < count($col_names); $k++) {
                                if ($col_names[$k] === $original_col_name || $col_names[$k] === $new_col_name) {
                                    $base_col_index = $k;
                                    break;
                                }
                            }
                            ...
                            if ($base_col_index === -1) {
                                ...
                                echo "    - 添加新列: {$new_col_name}\n";
                            } else {
                                // 如果列已存在，使用已有的列索引
                                $new_col_name = $col_names[$base_col_index];
                            }
                            
                            // 只在新列中添加数据，不修改原有列的数据
                            if (isset($current_row[$j]) && $base_col_index >= $colcount) {
                                // 只在新增的列中填入数据
                                $base_data[$i][$base_col_index] = $current_row[$j];
                                echo "    - 在新列 '{$new_col_name}' 添加值: " . $current_row[$j] . "\n";
                            } else if (isset($current_row[$j]) && $base_col_index < $colcount) {
                                // 如果是原有列，只在该列为空时更新
                                if (trim((string)$base_data[$i][$base_col_index]) === '') {
                                    $base_data[$i][$base_col_index] = $current_row[$j];
                                    echo "    - 更新空列 '{$new_col_name}' 的值: " . $current_row[$j] . "\n";
                                } else {
                                    echo "    - 跳过列 '{$new_col_name}'，基础表中已有值\n";
                                }
                            }
                        }
                    }
