<?php
/**
 * 修复版现代化Excel文件合并工具 (PHP版本)
 * 使用直接包含关键文件的方式解决服务器库不完整问题
 */
...
// 尝试引入完整的PhpSpreadsheet库 - 通过直接包含必要的文件
...
if (file_exists($vendor_path)) {
    ...
    if (!class_exists('PhpOffice\PhpSpreadsheet\IOFactory')) {
        exit("关键类 PhpOffice\\PhpSpreadsheet\\IOFactory 不存在\n");
    }
} else {
    exit("无法找到PhpSpreadsheet自动加载器: {$vendor_path}\n");
}

use PhpOffice\PhpSpreadsheet\IOFactory;

// 尝试读取基础文件
echo "尝试使用PhpSpreadsheet读取基础文件...\n";

try {
    $spreadsheet = IOFactory::load($base_file);
    ...
    for ($col = 1; $col <= $colcount; $col++) {
        ...
        $col_names[] = $worksheet->getCell($columnLetter . '1')->getValue();
    }
    ...
    for ($row = 2; $row <= $rowcount; $row++) {
        ...
        for ($col = 1; $col <= $colcount; $col++) {
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col);
            $row_data[] = $worksheet->getCell($columnLetter . $row)->getValue();
        }
        $base_data[] = $row_data;
    }
    ...
    exit("无法使用PhpSpreadsheet读取基础文件: " . $e->getMessage() . "\n");
}
...
foreach ($files_to_merge as $file_path) {
    ...
    try {
        // 读取当前文件
        $current_spreadsheet = IOFactory::load($file_path);
        $current_worksheet = $current_spreadsheet->getActiveSheet();
        
        // 获取当前文件的行数和列数
        $current_rowcount = $current_worksheet->getHighestRow();
        $current_colcount = $current_worksheet->getHighestColumn();
        $current_colcount = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($current_colcount);
        ...
        for ($col = 1; $col <= $current_colcount; $col++) {
            $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col);
            $current_col_names[] = $current_worksheet->getCell($columnLetter . '1')->getValue();
        }
        ...
        for ($row = 2; $row <= $current_rowcount; $row++) {
            ...
            for ($col = 1; $col <= $current_colcount; $col++) {
                $columnLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($col);
                $row_data[] = $current_worksheet->getCell($columnLetter . $row)->getValue();
            }
            ...
            if (!$match_found) {
                ...
                for ($j = 0; $j < count($current_col_names); $j++) {
                    ...
                    if ($j == $curr_name_col_index && $name_col_index != -1) {
                        $new_row[$name_col_index] = isset($current_row[$j]) ? (string)$current_row[$j] : '';
                    } elseif ($j == $curr_id_col_index && $id_col_index != -1) {
                        $new_row[$id_col_index] = isset($current_row[$j]) ? (string)$current_row[$j] : '';
                        ...
                        if ($base_col_index !== false) {
                            ...
                            // 添加新列
                            $col_names[] = $col_name;
                            $new_row[] = isset($current_row[$j]) ? (string)$current_row[$j] : '';
                        }
                    }
                }
