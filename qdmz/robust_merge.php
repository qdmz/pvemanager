...
// 尝试读取基础文件 - 增加调试信息
echo "尝试使用ExcelReader读取文件...\n";

$excel = null;
try {
    $excel = new ExcelReader();
    if (!$excel->read($base_file)) {
        echo "ExcelReader无法读取文件，尝试其他方法...\n";
    } else {
        echo "ExcelReader成功读取文件\n";
    }
} catch (Exception $e) {
    echo "ExcelReader读取文件时发生异常: " . $e->getMessage() . "\n";
    $excel = null;
}

// 检查Excel读取器是否成功加载文件
if (!$excel || !isset($excel->sheets) || !is_array($excel->sheets) || count($excel->sheets) == 0) {
    exit("Excel读取器无法读取基础文件: {$base_file}\n");
}
...
// 获取行数和列数
$rowcount = isset($sheet['numRows']) ? (int)$sheet['numRows'] : 0;
$colcount = isset($sheet['numCols']) ? (int)$sheet['numCols'] : 0;
...
foreach ($files_to_merge as $file_path) {
    ...
    // 检查Excel读取器是否正确加载
    if (!isset($current_excel->sheets) || !is_array($current_excel->sheets) || count($current_excel->sheets) == 0) {
        echo "  - Excel读取器未正确加载文件，跳过\n";
        continue; // 跳过无法读取的文件
    }
    
    // 获取当前Excel文件的行数和列数
    $current_sheet = $current_excel->sheets[0];
    $current_rowcount = isset($current_sheet['numRows']) ? (int)$current_sheet['numRows'] : 0;
    $current_colcount = isset($current_sheet['numCols']) ? (int)$current_sheet['numCols'] : 0;
