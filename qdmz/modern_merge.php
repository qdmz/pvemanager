...
try {
    ...
    for ($col = 1; $col <= $colcount; $col++) {
        $col_names[] = $worksheet->getCellByColumnAndRow($col, 1)->getValue();
    }
...
foreach ($files_to_merge as $file_path) {
    ...
    try {
        ...
        for ($col = 1; $col <= $current_colcount; $col++) {
            $current_col_names[] = $current_worksheet->getCellByColumnAndRow($col, 1)->getValue();
        }
        ...
        for ($row = 2; $row <= $current_rowcount; $row++) {
            $row_data = [];
            for ($col = 1; $col <= $current_colcount; $col++) {
                $row_data[] = $current_worksheet->getCellByColumnAndRow($col, $row)->getValue();
            }
