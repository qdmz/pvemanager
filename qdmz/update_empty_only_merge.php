<?php
/**
 * 更新空值版项目兼容Excel文件合并工具 (PHP版本)
 * 使用项目中已验证的Excel读取机制
 * 只更新基础表中为空的列数据，不覆盖已存在的数据
 */
                    ...
                    for ($j = 0; $j < count($current_col_names); $j++) {
                        if ($j != $curr_name_col_index && $j != $curr_id_col_index) { // 不处理姓名和身份证列
                            ...
                            if (isset($current_row[$j]) && 
                                (!isset($base_data[$i][$base_col_index]) || 
                                 trim((string)$base_data[$i][$base_col_index]) === '')) {
                                $base_data[$i][$base_col_index] = $current_row[$j];
                                echo "    - 更新列 '{$col_name}' 的值: " . $current_row[$j] . "\n";
                            } else {
