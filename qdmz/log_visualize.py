...
def log_message(message):
    ...
    # 同时写入日志文件
    log_file = "shujukufangzheli/analysis_log.txt"
...
def load_data():
    ...
    if csv_files:
        ...
        log_message(f"找到数据文件: {data_file}")
    else:
        log_message("未找到allinone.csv文件")
        return None
    
    try:
        df = pd.read_csv(data_file, encoding='utf-8-sig')
        log_message(f"成功加载数据，共 {len(df)} 行，{len(df.columns)} 列")
        log_message(f"列名: {list(df.columns)}")
        return df
    except Exception as e:
        log_message(f"加载数据时出错: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def analyze_subsidies(df):
    ...
    log_message(f"发现 {len(subsidy_cols)} 个可能的补贴相关列: {subsidy_cols}")
    ...
    if subsidy_cols:
        for col in subsidy_cols:
            ...
            log_message(f"{col}: {non_empty_count} 人获得补贴，总金额: {total_amount:.2f}，平均金额: {avg_amount:.2f}")
    
    return subsidy_cols

def analyze_age_groups(df):
    """分析年龄组分布"""
    log_message("开始分析年龄组分布...")
    ...
    log_message(f"年龄相关列: {age_cols}")
    log_message(f"出生日期相关列: {birth_cols}")
...
def analyze_gender(df):
    ...
    if gender_col and gender_col in df.columns:
        ...
        log_message(f"性别列: {gender_col}")
        log_message("性别分布:")
        for gender, count in gender_counts.items():
            percentage = (count / len(df)) * 100
            log_message(f"  {gender}: {count} 人 ({percentage:.2f}%)")
        return gender_counts
    else:
        log_message("未找到性别相关列")
        return None

def analyze_villages(df):
    """分析各村分布"""
    log_message("开始分析各村分布...")
    ...
    if village_col and village_col in df.columns:
        ...
        log_message(f"村庄列: {village_col}")
        log_message("各村分布:")
        for village, count in village_counts.items():
            percentage = (count / len(df)) * 100
            log_message(f"  {village}: {count} 人 ({percentage:.2f}%)")
        return village_counts
    else:
        log_message("未找到村庄相关列")
        return None
...
def create_visualizations(df):
    ...
    if subsidy_cols:
        try:
            ...
            if subsidy_names:
                ...
                plt.close()
                log_message(f"已生成补贴分布图: {os.path.join(output_dir, 'subsidy_distribution.png')}")
        except Exception as e:
            log_message(f"生成补贴分布图时出错: {e}")
    ...
    if gender_col and gender_col in df.columns:
        try:
            ...
            plt.close()
            log_message(f"已生成性别分布图: {os.path.join(output_dir, 'gender_distribution.png')}")
        except Exception as e:
            log_message(f"生成性别分布图时出错: {e}")
    ...
    if age_cols:
        try:
            ...
            if not age_group_counts.empty:
                ...
                plt.close()
                log_message(f"已生成年龄组分布图: {os.path.join(output_dir, 'age_group_distribution.png')}")
        except Exception as e:
            log_message(f"生成年龄组分布图时出错: {e}")
    ...
    if village_col and village_col in df.columns:
        try:
            ...
            plt.close()
            log_message(f"已生成村庄分布图: {os.path.join(output_dir, 'village_distribution.png')}")
        except Exception as e:
            log_message(f"生成村庄分布图时出错: {e}")
    ...
    try:
        ...
        plt.close()
        log_message(f"已生成数据概览图: {os.path.join(output_dir, 'data_overview.png')}")
    except Exception as e:
        log_message(f"生成数据概览图时出错: {e}")
    
    log_message(f"所有图表已保存到: {output_dir}")

def generate_detailed_report(df):
    """生成详细数据报告"""
    log_message("开始生成详细数据报告...")
    ...
    try:
        ...
        log_message(f"详细数据报告已保存到: {report_file}")
    except Exception as e:
        log_message(f"生成详细数据报告时出错: {e}")

def main():
    """主函数"""
    log_message("开始分析Excel数据...")
    
    # 加载数据
    df = load_data()
    if df is None:
        log_message("无法加载数据，程序结束")
        return
    ...
    log_message("数据分析完成！")
    log_message("生成的文件:")
    log_message("- 图表保存在: shujukufangzheli/charts/")
    log_message("- 报告保存在: shujukufangzheli/reports/")
    log_message("- 日志保存在: shujukufangzheli/analysis_log.txt")
