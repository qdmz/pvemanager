...
// 清空日志文件
file_put_contents($log_file, "开始合并Excel文件...\n", LOCK_EX);
...
log_message("检查目录: {$data_dir}");

// 检查目录是否存在
if (!is_dir($data_dir)) {
    log_message("错误: 目录 {$data_dir} 不存在");
    exit(1);
}
...
if (empty($excel_files)) {
    log_message("错误: 目录 {$data_dir} 中没有找到Excel文件");
    exit(1);
}

log_message("找到 " . count($excel_files) . " 个Excel文件");
...
if (!$base_file) {
    ...
    log_message("未找到'田冲2147总表'，使用 '" . basename($base_file) . "' 作为基础文件");
} else {
    log_message("使用 '" . basename($base_file) . "' 作为基础文件");
}

// 检查是否可以包含Excel读取器
$excel_reader_path = __DIR__ . '/inc/excel_reader.php';
if (!file_exists($excel_reader_path)) {
    log_message("错误: Excel读取器文件不存在: {$excel_reader_path}");
    exit(1);
}

log_message("包含Excel读取器文件");

// 包含Excel读取器
require_once $excel_reader_path;

log_message("尝试读取基础文件: " . basename($base_file));

// 读取基础文件
$excel = new ExcelReader();
if (!$excel->read($base_file)) {
    log_message("错误: 无法读取基础文件: {$base_file}");
    exit(1);
}

log_message("基础文件读取成功，共 {$excel->rowcount} 行，{$excel->colcount} 列");
...
log_message("基础文件列名: " . implode(", ", $col_names));
...
log_message("基础文件包含 " . count($base_data) . " 行数据");
...
if ($name_col_index == -1) {
    log_message("警告: 基础文件中未找到'姓名'列");
} else {
    log_message("姓名列索引: {$name_col_index}");
}

if ($id_col_index == -1) {
    log_message("警告: 基础文件中未找到'身份证'列");
} else {
    log_message("身份证列索引: {$id_col_index}");
}
...
log_message("需要合并 " . count($files_to_merge) . " 个文件");

// 合并其他文件的数据
foreach ($files_to_merge as $file_path) {
    log_message("正在处理文件: " . basename($file_path));
    
    // 读取当前文件
    $current_excel = new ExcelReader();
    if (!$current_excel->read($file_path)) {
        log_message("  - 无法读取文件，跳过: " . basename($file_path));
        continue; // 跳过无法读取的文件
    }
    
    log_message("  - 文件包含 {$current_excel->rowcount} 行，{$current_excel->colcount} 列");
    ...
    log_message("  - 当前文件列名: " . implode(", ", $current_col_names));
    ...
    if ($curr_name_col_index == -1 && $curr_id_col_index == -1) {
        log_message("  - 跳过文件，未找到姓名或身份证列: " . basename($file_path));
        continue; // 跳过没有姓名或身份证列的文件
    }
    
    log_message("  - 当前文件姓名列索引: {$curr_name_col_index}, 身份证列索引: {$curr_id_col_index}");
    
    // 合并数据
    foreach ($current_data as $current_row_idx => $current_row) {
        $match_found = false;
        ...
        log_message("    - 处理第 " . ($current_row_idx + 1) . " 行，姓名: {$name_value}, 身份证: {$id_value}");
...
if ($fp === false) {
    log_message("错误: 无法创建输出文件: {$output_file}");
    exit(1);
}
...
log_message("合并完成！结果已保存到: {$output_file}");
log_message("最终文件包含 " . count($base_data) . " 行数据");
log_message("最终文件包含 " . count($col_names) . " 列");

echo "\n处理完成！详细日志请查看 merge_log.txt 文件。\n";
?>